...
/*
	Init Разделительный пин просто от помех
*/
	RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOA, ENABLE);
...
void Power_off(uint8_t low_akb)
{
  static point_t pointXY;
	...
	if (low_akb==1){
	 LcdClear();
//   LcdGotoXYFont(4,3);
//   LcdStr(FONT_1X, "Низкий заряд АКБ");	
//   LcdGotoXYFont(7, 6);
//   LcdStr(FONT_1X, "Выключаюсь");			
		pointXY.x_point=2*11;
	  pointXY.y_point = 1*16;		
	  vram_put_str(pointXY, &tahoma_10ptFontInfo, "Низкий заряд", PIXEL_SET);		
	  pointXY.y_point = 2*16;
	  vram_put_str(pointXY, &tahoma_10ptFontInfo, "Выключаюсь", PIXEL_SET);
   ...
   //LcdStr(FONT_1X, "Выключаюсь");	
	 ...
	 LcdClear();
//   LcdGotoXYFont(4,3);
//   LcdStr(FONT_1X, "Сработал таймер");	
//   LcdGotoXYFont(7, 6);
//   LcdStr(FONT_1X, "Выключаюсь");	
		pointXY.x_point=6;
	  pointXY.y_point = 1*16;		
	  vram_put_str(pointXY, &tahoma_10ptFontInfo, "Сработал таймер", PIXEL_SET);	
		pointXY.x_point=2*11;		
	  pointXY.y_point = 2*16;
	  vram_put_str(pointXY, &tahoma_10ptFontInfo, "Выключаюсь", PIXEL_SET);		
	  LcdFillRect(1, 126, 1, 62, 1);	
